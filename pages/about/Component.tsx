...
function TextDisplay_({ temp, ...props }: { temp?: string }) {
	...
	return (
		<div {...props} onClick={() => setText("Text after click")}>
			{text}
		</div>
	);
}
...
function Toggle_({ temp, ...props }: { temp?: string }) {
	...
	return (
		<button type="button" {...props} onClick={() => setIsOn(!isOn)}>
			{isOn ? "ON" : "OFF"}
		</button>
	);
}
...
function CounterDoubler_({ temp, ...props }: { temp?: string }) {
	...
	return (
		<button type="button" {...props} onClick={() => setCount(count * 2)}>
...
function CurrencyConverter_({ temp, ...props }: { temp?: string }) {
	...
	return (
		<div {...props}>
			...
			/>
			<p>
				{amount} USD is equivalent to {convert()} EUR
			</p>
		</div>
	);
}
	...
	return (
		<div {...props}>
			<input
				type="number"
				value={celcius}
				onChange={(e) => setCelcius(Number(e.target.value))}
			/>
			<p>
				{celcius}°C is equivalent to {convert()}°F
			</p>
		</div>
	);
}
