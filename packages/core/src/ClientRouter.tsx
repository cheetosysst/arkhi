export class ClientRouter {
  ...
  private createLink: any = (url: string): void => {
    ...
    linkElement.onload = () => console.log('prefetched', url);
    linkElement.onerror = (err) => console.log('prefetched error', url, err);
      ...
      this.observer || (
        this.observer = new IntersectionObserver((entries, observer) => {
          ...
          entries.forEach((entry) => {
            const href: string = new URL(entry.target.getAttribute('href') || "", location.origin).href;
  ...
  private handleClientLinkBehavior(): void {
    document.querySelectorAll('a').forEach((element) => {
      element.addEventListener('click', (event: MouseEvent) => {
        ...
        this.updatePageContext({ href: target.getAttribute('href') || "", mode: 'go' });
      });
    });
  }
  ...
  private async updatePageContext({ href, mode, data }: { href: string, mode: string, data?: {} }): Promise<boolean> {
    ...
    try {
      ...
      if (mode === 'go') {
        window.history.pushState({ prev: window.location.href }, "", href);
      }
